...
bool nltokenBinaryPredicate(const nl::lex_token *first,
                            const nl::lex_token *second) {
  return *first == *second;
}

bool check_lexer(bool match, string input, vector<nl::lex_token> &expected) {
  ...
  if (match && result.size() == expected.size()) {
    ...
    auto diff = mismatch(result.begin(), result.end(), expected.begin(),
                         expected.end());
...
bool check_interpreter(string &program, string &expected) {
  string result = nl::run_interpreter(program);
  result = trim(result);
  if (result.compare(expected) != 0) {
    cout << "Interpretation has failed:" << endl << "[program]  `" << program
         << "`" << endl << "[expected] `" << expected << "`" << endl
...
bool check_parser(string input, vector<nl::lex_token *> &expected_lexer,
